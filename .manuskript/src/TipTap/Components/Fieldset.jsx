...
export default function Block({editor, node, updateAttributes, selected}) {
    ...
    return (
        ...
        >
            <div
                data-drag-handle
                className="flex cursor-grab items-center border-b border-slate-200 bg-slate-100 px-0.5 pt-1 pb-1.5"
            >
                <IconGripVertical className="mr-2" size={12} />
                <Label className={selected ? "font-semibold" : "font-normal"}>{label}</Label>
            </div>
            <Field className="w-full p-3" value={attrs.value} onChange={handleChange} {...props} />
        </NodeViewWrapper>
    );
}
